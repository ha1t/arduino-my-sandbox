...
const int cathode_pins[] = {0, 1, 2, 3, 4, 5, 6, 7};    // カソードに接続するArduinoのピン
const int anode_pins[] = {8, 9};  // コモンに接続するArduinoのピン
  ...
  for (int i = 0; i < number_of_anode_pins; i++) {
    ...
    for (int j = 0; j < number_of_cathode_pins; j++) {
      if (bitRead(digits[n % 10], j)) {
        digitalWrite(cathode_pins[j], PIN_OFF);
      } else {
        digitalWrite(cathode_pins[j], PIN_ON);
      }
    }

    digitalWrite(anode_pins[i], COMMON_ON);
    delayMicroseconds(300);
    digitalWrite(anode_pins[i], COMMON_OFF);
    n = n / 10; // 10で割る
  }
}
...
void gm34031_setup() {
  ...
  for (int i = 0; i < number_of_cathode_pins; i++) {
    ...
    digitalWrite(cathode_pins[i], PIN_OFF);
  }
